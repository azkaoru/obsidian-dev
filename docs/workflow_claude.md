# AIエージェント連携タスク管理ワークフロー（Claude Code版）

Obsidian vault 上でタスクを作成し、AIエージェント（Claude Code）が REST API 経由でタスクを取得・実行・完了するワークフローの説明です。
...
- Claude Code がインストールされていること（`claude` コマンドが使えること）
...
# または特定のディレクトリを指定して起動
claude --project /path/to/obsidian-dev
```
...
Claude Code を起動後（`$ claude`）、以下のように指示してください。
...
### ワンライナーで実行する場合（非対話モード）

```bash
# -p オプションで非対話モード（プロンプトを引数で渡す）
claude -p "get-task.sh を実行してタスクを取得し、プロンプトに従って作業後、complete-task.sh で完了してください。"
```
...
プロジェクトルートに `CLAUDE.md` を置くことで、Claude Code 起動時に自動的に読み込まれます。タスク管理ワークフローの共通ルールを記述しておくと便利です。

```markdown
# タスク管理ルール

- タスクは get-task.sh で取得すること
- 成果物は agentTasks/_ai_working/<yyyy-mm-dd>/ に保存すること
- 作業完了後は必ず complete-task.sh を実行すること
- OBSIDIAN_API_KEY 環境変数が設定されていることを確認すること
```

### 許可設定（.claude/settings.json）

よく使うコマンドを事前に許可しておくことで、実行時の確認プロンプトを省略できます。

```json
{
  "permissions": {
    "allow": [
      "Bash(bash get-task.sh)",
      "Bash(bash complete-task.sh *)",
      "Bash(curl -sk *)"
    ]
  }
}
...
| Claude Code が応答しない | モデルのレート制限 | しばらく待ってから再試行する |
| ツール実行の確認が多い | 許可設定が未構成 | `.claude/settings.json` に許可コマンドを追加する |
